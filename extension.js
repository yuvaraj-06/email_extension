...
gmail.observe.on("compose", function(compose,type) {
        var popup=null;
           ...
           error: function(jxhr){
				alert("turn on server")
         
            }
            });

        gmail.tools.add_compose_button(compose, "Drafty it!", function() {
			 
            if($('#review').is(":visible")==false && $('#improve_button').is(":visible")==false)
            {
		 
                //this inserts the sidebar on the right of the compose window
                if($('.GP').length == 0) {
                    alert("inserting");
                    $(popup).insertAfter('.et');
                } else {
                $(popup).insertBefore('.GP');
                }
			 
				var composeBody=''
					 
				var str1=compose.body();
				c=gmail.dom.visible_messages()
				console.log(c)
				alert(c)
					 
				composeBody=str1.slice(0);  
			
				$.post( "http://127.0.0.1:5000/postmethod", { data: composeBody },function(resultdata,status,xhr){
						//console.log("return value:"+resultdata)
						//var str12=compose.subject();
					//	alert(resultdata);
						scores=resultdata;
						
						
						updateUI(compose, JSON.parse(scores),popup,$);
				});
            ...
            {  
                ...
                //$('#improve_button').remove();
            }
     
        }); 
        //Set timeout is required as observe on compose event doesn't fire without it
...
updateUI= function(compose, scores,popup,$)
{   

	
    
    $('#complex_heading').html("<strong>Generated Mail </strong> <pre style='text-align: left;white-space:pre-line;'>"+scores.present+"</pre>" );

    
 
    
    //$('#word_count_heading').html("<strong>Length </strong>" + "  " + scores.vo);
    // var text = compose.body();
    // new_text=getHighlightedText(text, scores);

    // $('#text_analysis').html(new_text)

}
// getHighlightedText= function(text,scores)
// {

//     var mark_complex = '<mark style="background-color:#FFFFC2;">';
//     var mark_rude = '<mark style="background-color:#FBBBB9;">';
//     var mark_length = '<mark style="background-color:#C2DFFF;">';
//     var endmark = "</mark>";
//     var start_pos = 0;
//     var end_pos = 0;
//     var stop_word;
...
//     for (i = 0; i < scores.errors.length; i ++) {
//         stop_word = scores.errors[i][1];
//         start_word = scores.errors[i][2];
//         start_pos = text.indexOf(start_word, start_pos);
//         end_pos = String(text).indexOf(stop_word, start_pos);

//          if (scores.errors[i][0] == "rude") {

//             var output = [text.slice(0, start_pos), mark_rude, text.slice(start_pos)].join("");
//             var output1 = [output.slice(0, end_pos + 41), endmark, output.slice(end_pos + 41)].join("");
//             start_pos = end_pos + 48;

//         }

//         else if (scores.errors[i][0] == "length") {

//             var output = [text.slice(0, start_pos), mark_length, text.slice(start_pos)].join("");
//             var output1 = [output.slice(0, end_pos + 41), endmark, output.slice(end_pos + 41)].join("");
//             start_pos = end_pos + 48;

//         } else if (scores.errors[i][0] == "") {
//             start_pos = end_pos + 1;
//         }
//         text = output1;
//     }

//     if (scores.complex_list != []){
//         for (i = 0; i < scores.complex_list.length; i ++){
//             var startid = text.indexOf(scores.complex_list[i]);
//             var endid = startid + scores.complex_list[i].length;
//             var output = [text.slice(0, startid), mark_complex, text.slice(startid)].join("");
//             var output1 = [output.slice(0, endid + 40), endmark, output.slice(endid + 40)].join("");
//             text = output1;
//         }
//     }
